...
class AboutTrueAndFalse(Koan):
    ...
    def test_true_is_treated_as_true(self):
        self.assertEqual('true stuff', self.truth_value(True)) #true is true

    def test_false_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(False)) #false is false

    def test_none_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(None)) #none is false

    def test_zero_is_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value(0)) #0 is false

    def test_empty_collections_are_treated_as_false(self):
        self.assertEqual('false stuff', self.truth_value([]))
        self.assertEqual('false stuff', self.truth_value(())) #empty values are false
        self.assertEqual('false stuff', self.truth_value({}))
    ...
    def test_everything_else_is_treated_as_true(self):
        ...
        self.assertEqual('true stuff', self.truth_value([0])) #Other values are true
