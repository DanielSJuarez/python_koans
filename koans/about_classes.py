...
class AboutClasses(Koan):
    ...
    def test_instances_of_classes_can_be_created_adding_parentheses(self):
        # NOTE: The .__name__ attribute will convert the class #read!
        # into a string value.
        fido = self.Dog()
        self.assertEqual('Dog', fido.__class__.__name__)

    def test_classes_have_docstrings(self):
        self.assertRegex(self.Dog.__doc__, "Dogs need regular walkies. Never, ever let them drive.") #value of class Dog
    ...
    def test_init_method_is_the_constructor(self):
        dog = self.Dog2()
        self.assertEqual('Paul', dog._name) #set to the value of Dog2 
    ...
    def test_that_name_can_be_read_as_a_property(self):
        ...
        # access as method
        self.assertEqual('Fido', fido.get_name()) #method

        # access as property
        self.assertEqual('Fido', fido.name) #prop
    ...
    class Dog4:
        ...
        @property     #decorations that reference the method or function just previous to it
        def name(self):
            return self._name
    ...
    def test_different_objects_have_different_instance_variables(self):
        ...
        self.assertEqual(False, rover.name == fido.name) #values are not the same
