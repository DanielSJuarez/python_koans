...
class AboutLists(Koan):
    def test_creating_lists(self):
        ...
        self.assertEqual(0, len(empty_list)) #lenght equals 0 as there are not list objects
    ...
    def test_list_literals(self):
        ...
        nums[1:] = [2]
        self.assertListEqual([1, 2], nums) #add the value to the index shown

        nums.append(333)
        self.assertListEqual([1, 2, 333], nums) #appends is adding to the end of the list

    def test_accessing_list_elements(self):
        noms = ['peanut', 'butter', 'and', 'jelly']

        self.assertEqual('peanut', noms[0]) #using the indexvalue of the item and get its value
    ...
    def test_slicing_lists(self):
        ...
        self.assertEqual(['peanut'], noms[0:1]) #grabs the list item itself and add that many items based on second index value
    ...
    def test_slicing_to_the_edge(self):
        ...
        self.assertEqual(['and', 'jelly'], noms[2:]) #takes the initial index and the remaining items in the list
        self.assertEqual(['peanut', 'butter'], noms[:2]) #takes to items from the first index of the list as that index is not defined

    def test_lists_and_ranges(self):
        self.assertEqual(range, type(range(5)))
        self.assertNotEqual([1, 2, 3, 4, 5], range(1,6)) #range is displaying the index value that starts at 0 then goes to the range, if can also take an initial value to determin the starting index up until the range max which is the second value agruement
        self.assertEqual([0,1,2,3,4], list(range(5))) #''
        self.assertEqual([5,6,7,8], list(range(5, 9)))#''

    def test_ranges_with_steps(self):
        self.assertEqual([5,4], list(range(5, 3, -1))) #steps at incriments to the range on how much to step each time
    ...
    def test_insertions(self):
        ...
        knight.insert(2, 'not')
        self.assertEqual(['you', 'shall', 'not', 'pass'], knight) #insertions at the index passed to it
    ...
    def test_popping_lists(self):
        stack = [10, 20, 30, 40]
        stack.append('last') #append adds to the end
