...
class AboutClassAttributes(Koan):
    ...
    def test_objects_are_objects(self):
        fido = self.Dog()
        self.assertEqual(True, isinstance(fido, object)) #everything is an object
    ...
    def test_objects_have_methods(self):
        fido = self.Dog()
        self.assertEqual(26, len(dir(fido))) #length of the results, class has 26
    ...
    def test_creating_objects_without_defining_a_class(self):
        singularity = object()
        self.assertEqual(23, len(dir(singularity))) #class adds 3 so this is 26
    ...
    def test_other_objects_are_not_affected_by_these_singleton_functions(self):
        ...
        with self.assertRaises(Exception): rover.wag() #think about if it doesn't make sense it would throw an error
    ...
    def test_classmethods_can_not_be_used_as_properties(self):
        fido = self.Dog3()
        with self.assertRaises(TypeError): fido.name = "Fido" #the type is not equal thus a typeerror
    ...
    def test_classes_and_instances_do_not_share_instance_attributes(self):
        ...
        self.assertEqual('Rover', self.Dog3.get_name()) #seperate instance, seperate value
