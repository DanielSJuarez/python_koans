...
class AboutScope(Koan):
    ...
    def test_dog_is_not_available_in_the_current_scope(self):
        with self.assertRaises(Exception): fido = Dog() #error as there is not dogs available
    ...
    def test_you_can_reference_nested_classes_using_the_scope_operator(self):
        ...
        self.assertEqual(False, jims.Dog == joes.Dog) #to seperate values
    ...
    def test_constants_are_defined_with_an_initial_uppercase_letter(self):
        self.assertAlmostEqual(3.1416, self.PI) #upper case to notify the constance is just naming convension that the value shoudl not be changed
    ...
    def test_incrementing_with_local_counter(self):
        ...
        self.assertEqual(False, counter == start + 1) #value of counter is 2, and start is 3
    ...
    global deadly_bingo
    deadly_bingo = [4, 8, 15, 16, 23, 42]    #what you can do this, great! setting the global to the variable expands it to global scope
