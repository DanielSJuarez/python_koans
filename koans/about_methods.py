...
class AboutMethods(Koan):
    def test_calling_a_global_function(self):
        self.assertEqual(5, my_global_function(2,3)) #is like a function that is call. In this case taking the 2 agruments and completing a+b
    ...
    def test_calling_functions_with_wrong_number_of_arguments(self):
        ...
        self.assertRegex(msg,'my_global_function\(\) takes 2 positional arguments but 3 were given') #have to add the error message due to wrong number of arugments
    ...
    def test_calling_with_default_values(self):
        self.assertEqual([1, 'default_value'], self.method_with_defaults(1)) #since there was not a second parameter this simply grabs the default as the second arugment
    ...
    def test_calling_with_variable_arguments(self):
        ...
        self.assertEqual(('one', 'two'), self.method_with_var_args('one', 'two')) # *allows for an unknown amount of arguments to be passed in
    ...
    def test_functions_without_self_arg_are_global_functions(self):
        ...
        self.assertEqual(12, function_with_the_same_name(3,4)) #no self arugments are global thus take the default  for what is being passed

    def test_calling_methods_in_same_class_with_explicit_receiver(self):
        def function_with_the_same_name(a, b):
            return a * b

        self.assertEqual(7, self.function_with_the_same_name(3,4)) #because this cannot be access globally it will return the globally accessable def
    ...
    def test_that_old_methods_are_hidden_by_redefinitions(self):
        self.assertEqual(42, self.another_method_with_the_same_name()) #grabs the second def as that is hidden by the variable

    def test_that_overlapped_method_is_still_there(self):
        self.assertEqual(10, self.link_to_overlapped_method()) #is still maintained if that value is set
    ...
    def test_pass_does_nothing_at_all(self):
        ...
        "not"
        pass
        self.assertEqual(True, "Still got to this line" != None) #this works because cause it is just a function call and the value would still be return
    ...
    def test_no_indentation_required_for_one_line_statement_bodies(self):
        self.assertEqual('Madagascar', self.one_line_method()) #one-liner function
    ...
    def test_the_documentation_can_be_viewed_with_the_doc_method(self):
        self.assertRegex(self.method_with_documentation.__doc__,"A string placed at the beginning of a function is used for documentation") #you can put information with quote inside of the def
    ...
    class Dog:
        ...
        def _tail(self):
            # Prefixing a method with an underscore implies private scope #this is as naming convention to ensure that the scope of this informatoin is maintained
            return "wagging"
    ...
    def test_private_access_is_implied_but_not_enforced(self):
        ...
        self.assertEqual('wagging', rover._tail()) #this is rude because wagging is implided to be scoped

    def test_attributes_with_double_underscore_prefixes_are_subject_to_name_mangling(self):
        rover = self.Dog()
        with self.assertRaises(Exception): password = rover.__password() #name mangling is a replacement for the classname when the class has the same name. 
