...
class AboutGenerators(Koan):
    ...
    def test_generating_values_on_the_fly(self):
        ...
        self.assertEqual(['crunchy bacon','veggie bacon','danish bacon'], result) #loops through the array and adds the value to the end of each index

    def test_generators_are_different_to_list_comprehensions(self):
        num_list = [x*2 for x in range(1,3)]
        num_generator = (x*2 for x in range(1,3)) #generator is a replacement for a loop, also called yield
    ...
    def test_generator_expressions_are_a_one_shot_deal(self):
        ...
        self.assertEqual(['Boom!', 'Boom!', 'Boom!'], attempt1) #3 Booms as there are not booms
        self.assertEqual([], attempt2) #adds no booms since there are already booms present
    ...
    def test_generator_method_will_yield_values_during_iteration(self):
        result = list()
        for item in self.simple_generator_method(): #iterating over all the generators
            result.append(item) 
    ...
    def test_generators_can_be_manually_iterated_and_closed(self):
        ...
        self.assertEqual('peanut', next(result)) #not set to a list, and simple calling the first generators
        self.assertEqual('butter', next(result)) #'' but for the second generator
        result.close()
    ...
    def test_generator_method_with_parameter(self):
        ...
        self.assertEqual([4,9,16], list(result)) #saying for each value in the range square the value and return it 2*2=4, 3*3=9, etc
    ...
    def sum_it(self, seq):
        value = 0
        for num in seq:
            # The local state of 'value' will be retained between iterations #read this, then complete the calculation
            value += num
            yield value
    ...
    def test_generators_can_see_if_they_have_been_called_with_a_value(self):
        ...
        next(generator)
        self.assertEqual('with value', generator.send('with value'))#either have a value or no value
    ...
    def test_send_none_is_equivalent_to_next(self):
        ...
        self.assertEqual('no value', generator.send(None)) #none equals no value with generator
