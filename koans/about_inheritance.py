...
class AboutInheritance(Koan):
    ...
    def test_base_init_does_not_get_called_automatically(self): #is not setting the name in the class
        snoopy = self.Pug("Snoopy")
        with self.assertRaises(AttributeError): name = snoopy.name

    def test_base_init_has_to_be_called_explicitly(self): #specifically calling the name in the class
