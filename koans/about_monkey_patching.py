...
class AboutMonkeyPatching(Koan):
    ...
    def test_as_defined_dogs_do_bark(self):
        fido = self.Dog()
        self.assertEqual('WOOF', fido.bark()) #can calls the function
    ...
    def test_after_patching_dogs_can_both_wag_and_bark(self):
        ...
        fido = self.Dog()
        self.assertEqual('HAPPY', fido.wag()) #accesses the new method
        self.assertEqual('WOOF', fido.bark()) #access the the previous method
    ...
    def test_most_built_in_classes_cannot_be_monkey_patched(self):
        ...
        self.assertRegex(err_msg, "can't set attributes of built-in/extension type 'int'") #error message
